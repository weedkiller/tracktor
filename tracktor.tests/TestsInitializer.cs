...
{
    public class TestsInitializer : System.Data.Entity.DropCreateDatabaseAlways<TracktorContext>
    {
        protected override void Seed(TracktorContext context)
        {
            var user = new TUser { Name = "guest" };
            var intruderUser = new TUser { Name = "intruder" };
            context.TUsers.Add(user);
            context.TUsers.Add(intruderUser);
            context.SaveChanges();
